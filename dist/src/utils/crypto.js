...
export class Crypto {
    ...
    static aminoMarshalPubKey(pubKey, lengthPrefixed) {
        ...
        let pk = Utils.getAminoPrefix(pubKeyType);
    ...
    static getPrivateKeyFromKeyStore(keystore, password) {
        ...
        if (mac !== json.crypto.mac) {
            ...
            if (macLegacy !== json.crypto.mac) {
                throw new SdkError('Keystore mac check failed (sha3 & sha256) wrong password?', CODES.Internal);
            }
        }
     ...
     * @returns The private key
     */
    static getPrivateKeyFromKeystoreV1(keystore, password) {
        if (!is.string(password)) {
            throw new SdkError('No password given.', CODES.InvalidPassword);
        }
        // unarmor
        let keystore_content = Utils.unarmor(keystore);
        let header = keystore_content.header;
        if (!header.salt) {
            throw new SdkError('invalid keystore salt');
        }
        let salt = bcrypt.encodeBase64(Buffer.from(header.salt, 'hex'), 16);
        let key = bcrypt.hashSync(password, `${types.keystoreSaltPerfix}${salt}`);
        key = Utils.sha256(Buffer.from(key).toString('hex'));
        let keystoreData = Buffer.from(keystore_content.data, 'base64');
        const nonce = keystoreData.slice(0, types.xchacha20NonceLength);
        let privKey_full = nacl.secretbox.open(keystoreData.slice(types.xchacha20NonceLength), nonce, Buffer.from(key, 'hex'));
        if (!privKey_full) {
            throw new SdkError('KeyStore parsing failed', CODES.Internal);
        }
        let privKey = Buffer.from(privKey_full).slice(5).toString('hex');
