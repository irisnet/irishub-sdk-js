...
export class Utils {
    ...
    static unarmor(keystore) {
        ...
        if (keystoreTest.test(keystore)) {
            ...
            let datas = keystore.split('\n');
            if (!datas || !datas.length) {
                throw new SdkError('Invalid keystore', CODES.InvalidType);
            }
            datas = datas.filter((item) => item.trim().length);
            let headers = datas.filter((item) => item.indexOf(':') != -1);
            let contents = datas.filter((item) => item.indexOf(':') == -1);
            if (/^=/.test(contents[contents.length - 1])) {
                delete contents[contents.length - 1];
            }
            let header = Utils.parseKeystoreHeaders(headers);
     ...
     */
    static parseKeystoreHeaders(KeystoreHeaders) {
        let header = {};
        if (KeystoreHeaders && KeystoreHeaders.length) {
            KeystoreHeaders.forEach((item) => {
                let contents = item.split(':');
    ...
    static num2hexstring(num, size = 1, littleEndian = false) {
        if (typeof num !== 'number')
            throw new SdkError('num must be numeric');
        if (num < 0)
            throw new RangeError('num is unsigned (>= 0)');
        if (size % 1 !== 0)
            throw new SdkError('size must be a whole integer');
   ...
   */
    static getAminoPrefix(prefix) {
        let b = Array.from(Buffer.from((typeof SHA256 === 'function' ? SHA256 : SHA256.default)(prefix).toString(), 'hex'));
        while (b[0] === 0) {
            b = b.slice(1);
        }
        b = b.slice(3);
