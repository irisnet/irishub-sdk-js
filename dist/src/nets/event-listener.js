...
export class EventListener {
    ...
    newBlockHandler(callback, error, data) {
        ...
        if (blockData.block) {
            blockData.block.header.bech32_proposer_address = Crypto.encodeAddress(blockData.block.header.proposer_address, this.client.config.bech32Prefix.ConsAddr);
        }
        // Decode begin block tags
        if (blockData.result_begin_block) {
            blockData.result_begin_block.tags = Utils.decodeTags(blockData.result_begin_block.tags);
        }
        if (blockData.result_end_block) {
            // Decode end block tags
            blockData.result_end_block.tags = Utils.decodeTags(blockData.result_end_block.tags);
            ...
            if (blockData.result_end_block.validator_updates) {
                ...
                blockData.result_end_block.validator_updates.forEach((v) => {
                    ...
                    const bech32Pubkey = Crypto.encodeAddress(Crypto.aminoMarshalPubKey(valPubkey, false), this.client.config.bech32Prefix.ConsPub);
                    validators.push({
    ...
    newBlockHeaderHandler(callback, error, data) {
        ...
        // Decode proposer address
        blockHeader.header.bech32_proposer_address = Crypto.encodeAddress(blockHeader.header.proposer_address, this.client.config.bech32Prefix.ConsAddr);
        // Decode begin block tags
        if (blockHeader.result_begin_block) {
            blockHeader.result_begin_block.tags = Utils.decodeTags(blockHeader.result_begin_block.tags);
        }
        if (blockHeader.result_end_block) {
            // Decode end block tags
            blockHeader.result_end_block.tags = Utils.decodeTags(blockHeader.result_end_block.tags);
            // Decode validator updates
