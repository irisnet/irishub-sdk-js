  ...
  (0, _createClass2["default"])(EventListener, [{
    ...
    value: function unsubscribe(subscription) {
      ...
      this.wsClient.eventEmitter.on('unsubscribe#' + subscription.id, function (error, data) {
        ...
        _this7.wsClient.eventEmitter.removeAllListeners(subscription.id + '#event');
    ...
    value: function newBlockHandler(callback, error, data) {
      ...
      if (blockData.result_end_block) {
        ...
        blockData.result_end_block.tags = _utils.Utils.decodeTags(blockData.result_end_block.tags);
    ...
    value: function newBlockHeaderHandler(callback, error, data) {
      ...
      var blockHeader = data.data.value;
      // Decode proposer address
      blockHeader.header.bech32_proposer_address = _utils.Crypto.encodeAddress(blockHeader.header.proposer_address, this.client.config.bech32Prefix.ConsAddr);
      ...
      if (blockHeader.result_end_block) {
        // Decode end block tags
        blockHeader.result_end_block.tags = _utils.Utils.decodeTags(blockHeader.result_end_block.tags);
