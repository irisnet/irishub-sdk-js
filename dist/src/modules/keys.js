...
export class Keys {
    ...
    add(name, password, type = types.PubkeyType.secp256k1) {
        ...
        const address = Crypto.getAddressFromPublicKey(pubKey, this.client.config.bech32Prefix.AccAddr);
        const encryptedPrivKey = this.client.config.keyDAO.encrypt(privKey, password);
        const encryptedMnemonic = this.client.config.keyDAO.encrypt(mnemonic, password);
        let wallet = {
            address,
    ...
    recover(name, password, mnemonic, type = types.PubkeyType.secp256k1, index = 0, derive = true, saltPassword = '') {
        ...
        // }
        const privKey = Crypto.getPrivateKeyFromMnemonic(mnemonic, index, derive, saltPassword);
        const pubKey = Crypto.getPublicKeyFromPrivateKey(privKey, type);
        const address = Crypto.getAddressFromPublicKey(pubKey, this.client.config.bech32Prefix.AccAddr);
        const encryptedPrivKey = this.client.config.keyDAO.encrypt(privKey, password);
        const encryptedMnemonic = this.client.config.keyDAO.encrypt(mnemonic, password);
     ...
     * @since v0.17
     */
    importKeystore(name, password, keystore) {
        if (is.empty(name)) {
            throw new SdkError(`Name of the key can not be empty`);
        }
        if (is.empty(password)) {
        ...
        const address = Crypto.getAddressFromPublicKey(pubKey, this.client.config.bech32Prefix.AccAddr);
        const encryptedPrivKey = this.client.config.keyDAO.encrypt(pk.privKey, password);
        let wallet = {
            address,
            privateKey: encryptedPrivKey,
            publicKey: Crypto.aminoMarshalPubKey(pubKey),
     ...
     * @since v0.17
     */
    export(name, keyPassword, keystorePassword, iterations) {
        if (is.empty(name)) {
            throw new SdkError(`Name of the key can not be empty`);
        }
        ...
        const privKey = this.client.config.keyDAO.decrypt(keyObj.privateKey, keyPassword);
        const keystore = Crypto.generateKeyStore(privKey, keystorePassword, this.client.config.bech32Prefix.AccAddr, iterations);
